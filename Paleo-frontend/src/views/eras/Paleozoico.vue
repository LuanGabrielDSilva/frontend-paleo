      ...
      <div class="overlay">
        ...
        <div class="hero-meta">
          ...
          <div class="meta-item">
            ...
            <span class="meta-label">espécies surgidas</span>
          </div>
        </div>
      </div>
...
onMounted(async () => {
  try {
    const res = await fetch(
      'http://localhost:3333/periodos?eraId=b969ef71-0a80-46be-a8e6-f19094cfe698'
    )

    if (!res.ok) throw new Error('Falha na requisição')

    eras.value = await res.json()
  } catch (err) {
...
// Ordenação Cronológica Correta do Paleozoico
const sortedEras = computed(() => {
  ...
  // filtra apenas Paleozoico
  const paleozoicoOnly = eras.value.filter(
    era => era.era?.name === "Paleozoico"
  )

  return paleozoicoOnly.sort((a, b) => {
    const indexA = chronologicalOrder.indexOf(a.name)
    const indexB = chronologicalOrder.indexOf(b.name)
